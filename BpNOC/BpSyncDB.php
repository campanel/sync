...
class BpSyncDB {
    ...
    public function inicial_start()
    {
        ...
        /**
        Teste com apenas 10 ICs
        $arr = array();
        $i = 0;
        foreach($ics_opmon as $ic){
        if($i == 10 ){
        break;
        }
        $arr[] = $ic;
        ...
        foreach ($ics_opmon as $key => $ic) {
            $ic_priax_id = $bmPriax->get_priax_id_by_host_id($ic->host_id);
            if ($ic_priax_id) {
                BpSyncDB::insere_host($ic->host_id, $ic_priax_id, $ic->host_name);
                Bplog::save($ic_priax_id);
            } else {
                Bplog::save("Incliur a lista de delete IC", 1);
                $ics_delete[] = $ic;
            }
        }
        ...
        /**AICs **/
        foreach ($ics_opmon as $key => $ic) {
            $aics = BpNagiosServices::get_aics_by_host_id($ic->host_id);
            //var_dump($aics);
            if (count($aics) > 0) {
                foreach ($aics as $aic) {
                    $service_priax_id = $bmPriax->get_priax_id_by_service_id($aic->service_id);
                    var_dump("host_id:", $aic->host_id, "Service:", $aic->service_description, "id:", $aic->service_id, "priax_id", $service_priax_id);
                    if ($service_priax_id) {
                        BpSyncDB::insere_service($aic->service_id, $aic->host_id, $service_priax_id, $aic->service_description);
                    } else {
                        Bplog::save("Incliur a lista de delete AIC", 1);
                        $aics_delete[] = $aic;
                    }
                }
            } else {
        ...
        Bplog::save("ICs delete: " . count($ics_delete), 1);
        Bplog::save("AICs delete: " . count($aics_delete), 1);

        if (count($ics_delete) > 0 || count($aics_delete) > 0) {
            BpNagiosServices::delete_aics($aics_delete);
            BpNagiosHosts::delete_ics($ics_delete);
